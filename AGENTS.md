...
`data/builds/` contains 42 representative build JSON files (builds 01–42: the original 24-build cross-class set plus 3 additional Zealot fixtures, 3 additional Veteran fixtures, 3 additional Psyker fixtures, 3 additional Ogryn fixtures, 3 additional Arbites fixtures, and 3 additional Hive Scum fixtures) in canonical build shape. Each build stores `schema_version`, `title`, `class`, `provenance`, `ability`, `blitz`, `aura`, `keystone`, `talents[]`, `weapons[]`, and `curios[]`. Every selection carries `raw_label`, `canonical_entity_id`, and `resolution_status` (`resolved` / `unresolved` / `non_canonical`).

All 42 builds have been extracted from live GL pages with full talent trees, targeting Havoc 40 meta builds with diversity across keystones, weapons, and playstyles. Current fixture totals: 2255 resolved, 0 unresolved, 126 non_canonical. The checked-in fixture set no longer has any `unresolved` selections. The `non_canonical` bucket in the fixtures is six curio cosmetic base labels whose concrete runtime variants are collapsed by the scrape: `Blessed Bullet`, `Gilded Inquisitorial Rosette`, `Gilded Mandible`, `Guardian Nocturnus`, `Laurel of the Righteous`, and `Scrap of Scripture`. The runtime dump confirms 21 such ambiguous base labels in the full curio catalog.

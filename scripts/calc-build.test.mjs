...
const BUILD2 = join(__dirname, "builds", "01-veteran-squad-leader.json");
const SCRIPT = join(__dirname, "calc-build.mjs");
...
describe("calc CLI", { skip: !HAS_SOURCE && "requires GROUND_TRUTH_SOURCE_ROOT" }, () => {
  it("produces valid JSON with --json flag", () => {
    const out = execFileSync("node", [SCRIPT, BUILD, "--json"], {
      encoding: "utf-8",
      env: { ...process.env },
      timeout: 30000,
  ...
  it("produces text output by default", () => {
    const out = execFileSync("node", [SCRIPT, BUILD], {
      encoding: "utf-8",
      env: { ...process.env },
      timeout: 30000,
      maxBuffer: MAX_BUFFER,
  ...
  it("handles batch directory (text mode)", () => {
    const out = execFileSync("node", [SCRIPT, join(__dirname, "builds/")], {
      encoding: "utf-8",
      env: { ...process.env },
      timeout: 120000,
      maxBuffer: MAX_BUFFER,
  ...
  it("compare mode shows differences", () => {
    const out = execFileSync("node", [SCRIPT, BUILD, "--compare", BUILD2], {
      encoding: "utf-8",
      env: { ...process.env },
      timeout: 30000,
      maxBuffer: MAX_BUFFER,

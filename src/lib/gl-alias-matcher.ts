...
function exactAliasMatchesEntry(alias: AliasSchemaJson, entry: GlAliasCorpusEntry): boolean {
  ...
  if (entry.slot && !hasConstraint(alias, "slot", entry.slot)) {
    return false;
  }

  if (entry.class && !hasConstraint(alias, "class", entry.class)) {
    return false;
  }
...
const CURATED_GL_BLESSING_LABELS = new Map<string, string>([
  ...
  ["savage sweep", "shared.name_family.blessing.chained_hits_increases_melee_cleave"],

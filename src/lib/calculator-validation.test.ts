 ...
 * characterization for the damage pipeline across all checked-in builds.
 *
...
describe("Task 2: Damage Pipeline Sanity", { skip: !hasGeneratedData && "no generated data" }, async () => {
  ...
  it("runs computeBreakpoints on all checked-in builds (capturing errors)", () => {
    assert.ok(builds.length > 0, "expected at least one checked-in build fixture");
